...
#include "gfx/title_nam.h"
#include "gfx/level_nam.h"
#include "gfx/gameover_nam.h"
#include "gfx/welldone_nam.h"

//include nametables for levels

#include "gfx/level1_nam.h"
#include "gfx/level2_nam.h"
#include "gfx/level3_nam.h"
#include "gfx/level4_nam.h"
#include "gfx/level5_nam.h"
...
//sound effects
static Mix_Chunk* sfx_start;
static Mix_Chunk* sfx_item;
static Mix_Chunk* sfx_respawn1;
static Mix_Chunk* sfx_respawn2;

//sub songs in the music.ftm
static Mix_Music* music_level;
static Mix_Music* music_game;
static Mix_Music* music_clear;
static Mix_Music* music_game_over;
static Mix_Music* music_well_done;
static Mix_Music* music_lose;

void load_music() {
	Mix_AllocateChannels(PLAYER_MAX);

	music_level = Mix_LoadMUS("music/level.ogg");
	music_game = Mix_LoadMUS("music/game.ogg");
	music_clear = Mix_LoadMUS("music/clear.ogg");
	music_game_over = Mix_LoadMUS("music/gameover.ogg");
	music_well_done = Mix_LoadMUS("music/welldone.ogg");
	music_lose = Mix_LoadMUS("music/lose.ogg");

	sfx_start = Mix_LoadWAV("sfx/start.wav");
	sfx_item = Mix_LoadWAV("sfx/item.wav");
	sfx_respawn1 = Mix_LoadWAV("sfx/respawn1.wav");
	sfx_respawn2 = Mix_LoadWAV("sfx/respawn2.wav");
}

void unload_music() {
	Mix_HaltMusic();
	Mix_FreeMusic(music_level);
	Mix_FreeMusic(music_game);
	Mix_FreeMusic(music_clear);
	Mix_FreeMusic(music_game_over);
	Mix_FreeMusic(music_well_done);
	Mix_FreeMusic(music_lose);

	Mix_HaltChannel(-1);
	Mix_FreeChunk(sfx_start);
	Mix_FreeChunk(sfx_item);
	Mix_FreeChunk(sfx_respawn1);
	Mix_FreeChunk(sfx_respawn2);
}
...
Mix_Music** screenMusicList[3]={&music_level, &music_game_over, &music_well_done};
...
void show_screen(unsigned char num)
{
	...
	vram_adr(NAMETABLE_A);
	p3_write(screenList[spr], 1024);

	if(!spr)//if it is the level screen, print large number
	{
		j=num<<1;
		i16=0x0194;//position of the number in the nametable
	...
	pal_fade_to(4);
	Mix_PlayMusic(*screenMusicList[spr], 1);
...
void game_loop(void)
{
	...
	vram_adr(NAMETABLE_A);
	p3_write(levelList[i], 1024);				//unpack level nametable

	vram_adr(NAMETABLE_A+0x0042);
	vram_write((unsigned char*)statsStr,27); 	//add game stats string

	pal_bg((const char*) levelList[i+1]); 						//set up background palette
	pal_spr((const char*) palGameSpr); 							//set up sprites palette
	...
	while(!game_done)
	{
		...
		for(i=0;i<player_all;++i)
		{
			...
			{
				if(player_wait[i]==16)
					Mix_PlayChannel(i, i? sfx_respawn2 : sfx_respawn1, 0);
				--player_wait[i];
				continue;
			}
...
{
	// from crt0.s, init random generator
	set_rand(0xfdfd);
